...
def maintenance_list(request):
    ...
    pending_jobs = maintenances.filter(
        status="Pending"
    ).count()

    in_progress_jobs = maintenances.filter(
        status="In Progress"
    ).count()

    completed_jobs = maintenances.filter(
        status="Completed"
    ).count()

    critical_jobs = maintenances.filter(
        priority="Critical"
    ).count()
    ...
    total_cost = maintenances.aggregate(
        Sum("estimated_cost")
